...
impl GameEntity<([&mut Player; 2], &NPCList, &mut Stage, &BulletManager, &mut Flash, &mut NPCAccessToken)> for BossNPC {
    fn tick(
        &mut self,
        state: &mut SharedGameState,
        (players, npc_list, stage, bullet_manager, flash, token): (
            [&mut Player; 2],
            &NPCList,
            &mut Stage,
            &BulletManager,
